...
import logging
import warnings
from x_filter.utils import is_debug, calc_chunksize, fast_flatten, initializer
...
def filter_reference_BAM(bam, df, filter_conditions, threads, out_files, sort_memory):
    ...
    pysam.sort(
        ...
        str(sort_memory),
        "-o",
        out_files["x_filtered"],
        out_files["x_filtered_tmp"],
    )
    pysam.index(out_files["x_filtered"])
    os.remove(out_files["x_filtered_tmp"])
